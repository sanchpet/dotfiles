...
| `dot_local/bin/executable_guard-corporate-domain.tmpl` | `~/.local/bin/guard-corporate-domain` — PreToolUse hook denying the personal Claude profile any access to the employer's territory (work repos, work governance and knowledge, corporate hosts, the corporate Atlassian workspace) and pointing at `claude-work` instead. Territory comes from the same `work.gitdir` the git identity uses. A no-op under the work profile, blocking under every other one including the default — a wrong allow breaks company policy, a wrong block costs a restart. Every denial is appended to `~/.local/state/corporate-domain-guard.log` (paths and tool names only, never content), so the boundary leaves a record and not just a refusal |
