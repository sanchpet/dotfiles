...
- **Per-machine via `profile`, not per-machine directories.** One source tree; machine-specific
  ...
  everywhere, and an `includeIf "gitdir:~/IWE/employer-past/"` pulls in `dot_config/git/work.inc` to
  switch to the work identity (signing off) for the corporate repos under `~/IWE/employer-past/`.
  Defaulting to personal keeps the corporate email opt-in — it can only ever appear under
  `~/IWE/employer-past/` — and signing is gated on the key existing so a machine without it still
