...
**The agent answers without a live session** (`dev.sanchpet.dispositif.plist`): a login agent running [dispositif](https://github.com/sanchpet/dispositif), a mise tool, which polls the agent daemon, lets through only allowlisted messages, and answers each with one headless `claude -p` run whose capabilities depend on who wrote. The policy (allowlist, tiers, prompts) lives in the vault, not here. The guard keeps it inert on a machine without the vault, such as the work machine. Log: `~/Library/Logs/dispositif.log`.
