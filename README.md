...
- **Per-machine via `profile`, not per-machine directories.** One source tree; machine-specific
  ...
  everywhere, and an `includeIf "gitdir:~/hypomnemata/.repos/employer-past/"` pulls in
  `dot_config/git/work.inc` to switch to the work identity (signing off) for the corporate repos
  cloned under `~/hypomnemata/.repos/employer-past/` (the pre-migration `~/IWE/employer-past/` is still matched
