...
- **Disclose AI in the PR/MR description, on every PR whose description an agent wrote or edited** — one `Assisted-By: Claude <noreply@anthropic.com>` line at the end, not a tooling banner like `🤖 Generated with Claude Code`. Default-*on* everywhere, and the contrast with the trailer above is deliberate rather than inconsistent: a trailer is immutable history and an unwanted one cannot be recalled, while a description is editable prose that costs nothing to correct. What it buys is the thing the trailer cannot — the reader of that text is a person, and taking agent-written prose for a colleague's is exactly the misreading worth preventing. Still no attribution line in committed docs or code comments: there the trailer on the commit carries it. A target project whose policy forbids AI mentions wins over this (see the contributing rule below).
