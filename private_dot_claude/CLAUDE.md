...
- **Disclose AI in review prose an agent wrote — the PR/MR description, and any comment it posts in the thread.** One `Assisted-By: Claude <noreply@anthropic.com>` line at the end, never a tooling banner like `🤖 Generated with Claude Code`. Default-*on* everywhere, and the contrast with the trailer above is deliberate rather than inconsistent: a trailer is immutable history and an unwanted one cannot be recalled, while review prose is editable and costs nothing to correct. What it buys is the thing the trailer cannot — that text is read by a person, and taking agent-written prose for a colleague's is exactly the misreading worth preventing. Mind the scope: prose in the review venue, not in the repository — committed docs and code comments carry no attribution line, the commit trailer covers those. A target project whose policy forbids AI mentions wins over this (see the contributing rule below).
