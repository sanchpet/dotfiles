...
- **Disclose AI authorship where the venue asks for it — not by default.** Before writing prose a person will read (a PR/MR description, an issue or bug report, a comment in either thread), check the target's `CONTRIBUTING` / AI policy: some projects require a declaration, some forbid AI mentions outright, most say nothing. Where nothing is required, add no attribution line — the owner publishes under their own account and owns what it says. This governs whether to *volunteer* a footer, not whether to be truthful: never deny AI involvement when someone asks directly. Where a declaration is required, give it in the venue's own form, once, at the end — never a tooling banner like `🤖 Generated with Claude Code`. Scope is prose in the venue, not the repository: committed docs and code comments carry no attribution line, the commit trailer above covers those.
