...
- **Length scales with the change, not with what you learned making it.** A one-file diff gets a short description; four paragraphs on it read as padding whatever they contain. Cut every sentence a teammate would supply themselves — shared context is theirs already, and restating it is the commonest form of slop. What survives is what they could not have known: the constraint from outside the diff, the option rejected and why, the dependency on another change.
...
- **AI attribution trailer:** `Assisted-By: Claude <noreply@anthropic.com>` — accurate by design (the owner authors, Claude assists), unlike `Co-Authored-By`, which asserts joint authorship and inflates the contributor graph. **Add it only when the remote is one of the owner's own repos (`github.com/sanchpet/*`), or when the target project's policy asks for it (see the contributing rule below). Everywhere else: no trailer.** Default-off because the failure is asymmetric — a missing trailer on a personal repo is a footnote, while an AI trailer pushed into an employer's history cannot be recalled. It also means no list of employer hosts has to be recognised: an unfamiliar remote simply gets nothing, including when the agent runs outside the vault where that list lives.
- **Disclose AI in prose an agent wrote for a human channel — a PR/MR description, an issue or bug report, and any comment in either thread.** That list is illustrative, not exhaustive: the test is whether a person will read the text and could take it for the owner's own writing. One `Assisted-By: Claude <noreply@anthropic.com>` line at the end, never a tooling banner like `🤖 Generated with Claude Code`. Default-*on* everywhere, and the contrast with the trailer above is deliberate rather than inconsistent: a trailer is immutable history and an unwanted one cannot be recalled, while such prose is editable and costs nothing to correct. What it buys is the thing the trailer cannot — that text is read by a person, and taking agent-written prose for a colleague's is exactly the misreading worth preventing. Mind the scope: prose in the venue, not in the repository — committed docs and code comments carry no attribution line, the commit trailer covers those. A target project whose policy forbids AI mentions wins over this (see the contributing rule below).
...
- **A repo's own `CLAUDE.md` is read by every contributor's agent, not just yours.** In a public repo it may state only what holds for any contributor — branch + PR, commit format, how to run the tests. Never put merge authority, standing grants, or anything else owner-specific there: an agent told "merging is delegated to you" will walk a branch it has no rights to merge, and may act as though it holds authority it doesn't. Say "branch + PR" and stop — who merges depends on the reader, which that file cannot know. Personal authority lives here and in the vault.
...
- **A comment earns its lines, and the budget is small.** Default to none: a colleague on the team reads the same code and knows the same conventions, so anything they would infer unaided is noise. Write one only where they would genuinely stall — a non-obvious choice, a constraint from outside the file, a workaround whose reason is invisible. Then keep it to a sentence or two: *why*, and under which ticket. A block of five-plus lines is a **slop grenade** — it looks like diligence and gets skimmed, which means the one line that mattered is skimmed with it. Same reader, same failure mode as the AI wall in a PR description: length destroys the attention it was meant to buy. If the rationale genuinely needs paragraphs, its home is the MR description or the ticket, not the file.
